...
const ChatInput = ({ chatId, messages, setMessages, setChats,setIsTyping }) => {
...
typingTimeOutRef.current = setTimeout(() => {
  ...
  const sendMessage = () => {
    ...
    setChats((prevChats) =>
      prevChats.map((chat) =>
        chat.id === chatId ? { ...chat, lastMessage: input, time:timestamps, online: true } : chat
      )
    );
  ...
  return (
    <div className="p-4 border-t flex items-center space-x-2 relative">
      ...
      <label htmlFor="fileInput" className="cursor-pointer text-xl">📎</label>
      <input type="file"
      id="fileInput"
      className="hidden"
      onChange={(e)=>{
